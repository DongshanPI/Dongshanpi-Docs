...
DongshanPI-PicoW属于最小开发板，可以直接使用 5V电压输入 即可开始工作,可以参考下图所示，使用 串口 模块 分别连接` P42 TX`连接至USB串口模块`RX引脚`，`P43 RX`连接至串口模块 `TX引脚`，`P46 GND`连接至串口模块 `GND`,另外  可以选择 `P31 5V`连接至串口模块 `5V`电压输出，确认好连接后，可以通电连接至 电脑USB接口，板子会默认启动，可以通过串口工具等 打开串口 进入系统。
